...
export async function GET() {
  try {
    const imports = await prisma.iMPORT.findMany({
        ...
        orderBy: {
            created_at: "desc", // Sắp xếp theo ngày tạo giảm dần
        },
    });
...
export async function POST(req: Request) {
    try {
        ...
        const totalPrice = products.reduce(
            (sum: number, product:any) => sum + Number(product.price) * Number(product.quantity),
            0
        );
        ...
        return NextResponse.json({message: "Thêm đơn nhập hàng thành công", data: newImport});
    } catch (error) {
        console.error("Lỗi khi thêm đơn nhập hàng:", error);
        return NextResponse.json({ message: "Lỗi khi thêm đơn nhập hàng!" }, { status: 500 });
    }
}