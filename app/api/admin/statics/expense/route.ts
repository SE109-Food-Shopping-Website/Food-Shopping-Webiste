...
export async function GET(req: NextRequest) {
  try {
    ...
    const currentOrders = await prisma.iMPORT.findMany({
      where: {
        updated_at: {
          gte: currentStart,
          lte: currentEnd,
    ...
    const previousOrders = await prisma.iMPORT.findMany({
      where: {
        updated_at: {
          gte: prevStart,
          lte: prevEnd,
    ...
    // Tính sự thay đổi phần trăm giữa tiền chi tháng hiện tại và tháng trước
    ...
    console.error(error);
    return NextResponse.json({ error: "Lỗi khi tính tiền chi!" }, { status: 500 });
  }
}
